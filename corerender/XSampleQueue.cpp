...
struct _rbuf_s {
    ...
    rbuf_mode_t mode;           // the ringbuffer mode (blocking/overwrite)
};
...
void rbuf_skip(rbuf_t *rb, int size) {
...
int rbuf_read(rbuf_t *rb, u_char *out, int size) {
...
int rbuf_write(rbuf_t *rb, u_char *in, int size) {
...
int rbuf_size(rbuf_t *rb) {
    return rb->size;
}

int rbuf_available(rbuf_t *rb) {
...
int rbuf_find(rbuf_t *rb, u_char octet) {
    int i;
...
int rbuf_read_until(rbuf_t *rb, u_char octet, u_char *out, int maxsize) {
    int i;
    int j = 0;
    int size = rb->used;
    int to_read = size;
...
static int rbuf_copy_internal(rbuf_t *src, rbuf_t *dst, int len, int move) {
    if (!src || !dst || !len)
        return 0;
